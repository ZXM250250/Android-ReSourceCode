 ...
 */
//源码第一部分总结  第一  RecycleView的绘制流程的总结
/**
 * 1.RecyclerView是将绘制流程交给LayoutManager处理，如果没有设置不会测量子View。
 * 2.绘制流程是区分正向绘制和倒置绘制。
 * 3.绘制是先确定锚点，然后向上绘制，向下绘制，fill()至少会执行两次，
 * 如果绘制完还有剩余空间，则会再执行一次fill()方法。
 * 4.LayoutManager获得View是从RecyclerView中的Recycler.next()方法获得，涉及到RecyclerView的缓存策略，
 * 如果缓存没有拿到，则走我们自己重写的onCreateView方法。
...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        if (mLayout == null) {
            /**
             * layoutManager没有设置的话，直接走default的方法，
             * 所以会为空白(这里实际上，并没有对子view进行测量)
             */
        ...
        if (mLayout.mAutoMeasure) {
            ...
            /**
             * 如果测量是绝对值，则跳过measure过程直接走layou
             *
             * onLayout里会执行子View的绘制
             */
    ...
    void dispatchLayout() {
        ...
        /**
         当我们给RecyclerView设置固定的宽高的时候，onMeasure
         是直接跳过了执行，那么为什么子View仍然能绘制出来。
         这里可以看到，如果onMeasure没有执行，
         mState.mLayoutStep == State.STEP_START就成立，
         所以仍然会执行 dispatchLayoutStep1()，
         dispatchLayoutStep2();也就对应的会绘制子View。
         */
        if (mState.mLayoutStep == State.STEP_START) {
            dispatchLayoutStep1();
        ...
        /**
         * dispatchLayoutStep3()方法的作用除了重置一些参数，
         * 外还和执行动画有关。
         */
        dispatchLayoutStep3();
    }
    ...
    private void dispatchLayoutStep2() {
        ...
        mState.mInPreLayout = false;
        /**
         * 在这里就是 RecycleView将View的绘制
         * 交给了LayoutManger  直接证明
         */
        mLayout.onLayoutChildren(mRecycler, mState);
    ...
    Rect getItemDecorInsetsForChild(View child) {
        ...
        for (int i = 0; i < decorCount; i++) {
             ...
             * 这里其实也就可以理解了自定义分割线的原理就是在子
             * View的测量过程前给上下左右加上自定义分割线所对应设置
             * 给这个child的边距。
             */
     ...
     */
    /**
     * 可以看到RecyclerdViewPool内部使用到了Google
     * 推荐的数据结构类型SparseArray，而SparseArray内部的key
     * 就是我们的ViewType，而value存放的是ArrayList<ViewHolder>。
     * 而默认的每个ArrayList<ViewHolder>的大小是5个。
     * 这里还有一个要注意的点就是getRecycledView这个方法可以看到拿
     * 到viewholder其实是通过remove拿到的，也就是通过remove拿到的。
     *
    ...
    /**
     * 这是一个很清晰的类结构
     * 这里可以清楚的看到我们后面讲到的四级缓存机制所用到的类都在这里可以看到：
    ...
    public final class Recycler {
         ...
         */
        /**
         * 注释写的很清楚，从Recycler的scrap，
         * cache，RecyclerViewPool,或者直接create创建
         *
         *
         * 1.RecyclerView内部大体可以分为四级缓存：
         * mAttachedScrap,mCacheViews,ViewCacheExtension,
         * RecycledViewPool.
         * 2.mAttachedScrap,mCacheViews在第一次尝试的时候只是对View的复用，
         * 并且不区分type，但在第二次尝试的时候是区分了Type，
         * 是对于ViewHolder的复用，ViewCacheExtension,RecycledViewPool
         * 是对于ViewHolder的复用，而且区分type。
         * 3.如果缓存ViewHolder时发现超过了mCachedView的限制，
         * 会将最老的ViewHolder(也就是mCachedView缓存队列的第一个
         * ViewHolder)移到RecycledViewPool中。
         *
        。
         */
        @Nullable
        ViewHolder tryGetViewHolderForPositionByDeadline(int position,
            ...
            if (mState.isPreLayout()) {
                /**
                 * preLayout默认是false，只有有动画的时候
                 * 才为true
                 * getChangedScrapViewForPosition方法，
                 * 其实是从Recycler类中的mChangedScrap获取ViewHolder
                 */
            ...
            /**
             * 第一级缓存，可以看到，
             * 这里通过getScrapOrHiddenOrCachedHolderForPosition
             * 方法来获取ViewHolder，并检验holder的有效性，如果无效，
             * 则从mAttachedScrap中移除，并加入到mCacheViews或者Pool中，
             * 并且将holder至null，走下一级缓存判断。
             */
             ...
             *
             * mAdapter.getItemViewType(offsetPosition);熟悉的方法有木有，
             * 可以看到这里调用了我们平常使用RecyclerView进行多样式item的方法，
             * 也就是说前面对于一级缓存mAttachedScrap和mCacheViews是不区分type的，
             * 从现在开始的判断是区分type的缓存。这里对于我们研究多type类型的
             * RecyclerView很有帮助。
             * 接下来的判断可以看到很明显这是对于我们重写hasStableIds()
             * 方法为true的情况。
             *
             */
            if (holder == null) {
                ...
                if (holder == null) {
                    ...
                    /**
                     * 在经过一堆的判断方法以后  如果都不成立 那么就走到了这里
                     * 这里也到了我们熟悉的 createViewHolder方法
                     *
                     *
                     * 当所有的的尝试从缓存中获取都失败后，
                     * 只能调用我们自己重写的createViewHolder方法，重新创建一个
                     */
        ...
        ViewHolder getScrapOrHiddenOrCachedHolderForPosition(int position, boolean dryRun) {
            ...
            if (!dryRun) {
                ...
                if (view != null) {
                    ...
                    /**
                     * 添加到Scrap中，其实这里既然已经拿到
                     * 了ViewHolder，可以直接传vh进去
                     */
                    scrapView(view);
     ...
     * 自定义缓存的类
     * 1.首先这个类基本上没有什么限制，
     * 也就是说无论是缓存使用的数据结构还有缓存算法(LRU还是什么)
     * 完全自定义，都由开发者自己决定，
     * 这一点可以说既给了开发者很大的便利，也给开发者带来了很大的隐患。
     * 2.对于平常的缓存，我们的理解在怎么说至少get-add|push-pop
     * 都是成对出现，为什么这样说的，也就是缓存至少有进也有出。
     * 而这里可以看到这里的抽象类只定义了出的方法，也就是只出不进，
     * 进的时机，大小，时效等完全没有规定。

     */
    public abstract static class qViewCacheExtension {
    ...
    public abstract static class LayoutManager {
        ...
        public void measureChildWithMargins(View child, int widthUsed, int heightUsed) {
            ...
            /**
             * 设置分割线中的回调方法
             * getItemDecorInsetsForChild（）这个方法值得注意
             */
            final Rect insets = mRecyclerView.getItemDecorInsetsForChild(child);
            widthUsed += insets.left + insets.right;
            heightUsed += insets.top + insets.bottom;
