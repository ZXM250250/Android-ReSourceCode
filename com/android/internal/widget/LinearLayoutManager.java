...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        //判断绘制方向,给mShouldReverseLayout赋值,
        // 默认是正向绘制，则mShouldReverseLayout是false
        /**
         * 询问是否需要倒着绘制
         */
        resolveShouldLayoutReverse();
        //mValid的默认值是false，一次测量之后设为true，
        // onLayout完成后会回调执行reset方法，又变为false
        /**
         * mAnchorInfo  就是说的 锚点
         * 变量的解释：mValid的默认值是false，
         * 一次测量之后设为true，onLayout完成后会回调执行reset方法，
         * 又变为false
         */
        if (!mAnchorInfo.mValid || mPendingScrollPosition != NO_POSITION
                || mPendingSavedState != null) {
            mAnchorInfo.reset();
            //mStackFromEnd默认是false，
            // 除非手动调用setStackFromEnd()方法，两个都会false，
            // 异或则为false
            /**
             *  mAnchorInfo.mLayoutFromEnd  默认的结果  是false
             */
        ...
        if (mAnchorInfo.mLayoutFromEnd) {
            // fill towards start
            /**
             * 正常绘制的话，updateLayoutStateToFillStart(mAnchorInfo);
             * 先往上绘制
             * ，updateLayoutStateToFillEnd(mAnchorInfo);再往下绘制
             * updateLayoutStateToFill...()目的就是
             * 为了确定当前方向上 锚点的相关的状态信息
             *
             */
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        /**
         *在执行完addView（）的方法以后  就会执行
         * measureChildWithMargins(view, 0, 0);
         * 进行子View的测量    不过这都是RecyclerView的方法
         * 源码 去RecyclerView里面点
         */
        measureChildWithMargins(view, 0, 0);
    ...
    static class LayoutState {
        ...
        View next(RecyclerView.Recycler recycler) {
            //默认mScrapList=null，
            // 但是执行layoutForPredictiveAnimations方法的时候不会为空
            if (mScrapList != null) {
                return nextViewFromScrapList();
            }
            //重要，从recycler获得View,mScrapList是被LayoutManager持有，
            // recycler是被RecyclerView持有
            /**
             * 这里也就是缓存策略的身影
             * 重要，从recycler获得View,mScrapList是被LayoutManager持有，
             * recycler是被RecyclerView持有
